...
4. Copy files and commit:
   ...
   The checked-in `.SRCINFO` is generated by the project release flow, so `makepkg` is not required just to sync AUR metadata.
   The AUR remote only accepts pushes to `master`, so automated release syncs use that branch even if your local checkout started elsewhere.
